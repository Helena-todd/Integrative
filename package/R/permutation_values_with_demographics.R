...
perm_val_demo <- function (data, nb_perm, ind_feature){
  ...
  for(i in 1:(nb_perm-1)){
    ...
    fit <- nnet::multinom(group ~ gender + age, data=data_tmp,trace=F)
    orig_AUC <- pROC::multiclass.roc(data_tmp$group,fitted(fit))$auc[[1]]
  ...
  fit_ag <- nnet::multinom(group ~ gender + age, data=data_tmp,trace=F)
  orig_AUC <- pROC::multiclass.roc(data_tmp$group,fitted(fit_ag))$auc[[1]]
