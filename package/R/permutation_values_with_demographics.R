...
perm_val_demo <- function (data, nb_perm, ind_feature, orig_AUC){
  ...
  data_tmp <- data %>%
    mutate(gender = as.factor(big_mat$GENDER),
           cmv = as.factor(big_mat$CMVStatus))
  ...
  for(i in 1:(nb_perm-1)){
    ...
    fit2 <- nnet::multinom(group ~ eval(parse(text=colnames(data)[ind_feature])) + gender + cmv, data=data_tmp,trace=F)
  ...
  fit_demo <- nnet::multinom(group ~ eval(parse(text=colnames(data)[ind_feature])) + gender + cmv, data=data_tmp,trace=F)
  ...
  return(lst(quantile = qt, quantile_demo = qt_demo))
}
