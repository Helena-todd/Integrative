#' Gather Donors, Recipients and D-R
#'
...
#' @param data_donors The path to the matrix with donors in rows and selected feaatures in columns
#' @param beh_donors The path to the list of donor features that had the same behaviour in the 2 cohorts
...
gather_DRDR <- function(data_couples, beh_couples = NULL,
                        data_recip, beh_recip = NULL,
                        data_donors, beh_donors = NULL,
  ...
  # donor info :
  load(data_donors)
  if(!is.null(beh_donors)){
    # select only the features that had the same behaviour in the two cohorts
    sel_beh <- read.xlsx(beh_donors)
    pctgs_sel_ft_donor_90 <- pctgs_sel_ft_donor_90[,c("group", "Id.Cryostem.R", sel_beh$sel_beh)]
  }
