...
```{r, echo = FALSE}
couple_info <- colData %>% 
  filter(COUPLENUMBER %in% complete_couples) %>% 
...
tolerant recipients, with a 0.95 threshold:
```{r, echo = FALSE}
qt <- unlist(map(perm_vals, 1))
densityplot(qt)
selected_genes_qt <- colnames(norm_data)[which(qt>.95)+1]
write.xlsx(selected_genes_qt, file = "../data/RNAseq/recip_only/perm_val_TNT_95_thresh.xlsx")
print(paste0(length(selected_genes_qt),
             " genes were selected with a 0.95 threshold in the St Louis cohort."))
```
...
I save the 140 features in an excel file:
...
```{r, echo = FALSE}
rna_recip_louis_TNT_95 <- norm_data[,c("group", selected_genes_qt[sel_common])]
save(rna_recip_louis_TNT_95,
     file = "../data/RNAseq/recip_only/rna_recip_louis_TNT_95.RData")
...
write.xlsx(annot_genes, file = "../data/RNAseq/recip_only/annot_selected_bis_genes_95.xlsx")
```
...
write.xlsx(dir_genes, file = "../data/RNAseq/recip_only/perm_val_dir_TNT_95_thresh.xlsx")
```

Graph of the 111 selected genes that were found commonly between the two cohorts:
```{r, echo = FALSE}
sel_beh <- read.xlsx("../data/RNAseq/recip_only/perm_val_beh_TNT_95_thresh.xlsx")
...
```{r, echo = FALSE}
rna_recip_louis_beh_TNT_95 <- norm_data[,c("group", selb)]
save(rna_recip_louis_beh_TNT_95,
     file = "../data/RNAseq/recip_only/rna_recip_louis_beh_TNT_95.RData")
...
# for( i in which(comp_sizes != 0)){
#   vertex_names <- vertex_attr(gr_components[[i]], index = V(gr_components[[i]]))[[1]]
#   vertex_membership <- as.numeric(selected_genes_qt[sel_common] %in% vertex_names)
#   annot_genes <- cbind(annot_genes, vertex_membership)
# }
...
write.xlsx(annot_genes, file = "../data/RNAseq/recip_only/annot_111selected_genes_recip_TNT_95.xlsx")
```
...
```{r, echo = FALSE}
set.seed(1)
rf <- ranger::ranger(group~., data = rna_recip_louis_TNT_95, num.trees = 5000, importance = "impurity")
rf$confusion.matrix
...
the RF model built on the 111 selected features isn't better than the one built 
on the 140 common genes:
```{r, echo = FALSE}
set.seed(1)
rf <- rf_tol_non_tol(df_orig = rna_recip_louis_beh_TNT_95, ntree = 5000)
rf
```
...
We can isolate the genes that were selected as informative to classify the non and 
tolerant recipients, after correcting for age and gender, with a 0.90 threshold:
```{r}
qt_bla <- unlist(map(perm_vals, 2))
densityplot(qt_bla)
selected_bla <- colnames(norm_data)[which(qt_bla>.90)+1]
write.xlsx(selected_bla, file = "../data/RNAseq/recip_only/perm_val_TNT_gendage_90_thresh.xlsx")
```



Of the 408 genes that were selected as informative on tolerance in both cohorts, 
...
Some genes (end of the second "bump" in the density plot) seem to still add 
information on the tolerance even after correction for gender and age:
```{r}
...
sel_com_names[which(auc_g > .74)]
```

Boxplots of the most gender related genes:
```{r, echo = FALSE}
toplot <- norm_data[,c("gender_comp", sel_com_names[which(auc_g > .74)])]
for(i in seq_along(sel_com_names[which(auc_g > .74)])){
...
which(auc_a<20000000)
```

We can plot the genes that seemed to be most linked to the age of the recipients:
```{r, echo = FALSE}
toplot <- norm_data[,c("age_recip", "group", sel_com_names[which(auc_a < 20000000)])]
toplot$age_recip <- round(toplot$age_recip/365)
for(i in seq_along(sel_com_names[which(auc_a < 20000000)])){
...
Of the 45 genes that were selected as informative on tolerance in both cohorts, 
which ones are still high after correcting for age and gender?
```{r, echo = FALSE}
sel_com_names <- selected_genes_qt[sel_common]
...
A few of the selected genes (second "bump" in the density plot) still seem to 
add information on the tolerance even after correction for gender and age:
```{r, echo = FALSE}
names(qt_sel)[which(qt_sel>0.5)]
order_demo_corr <- sel_com_names[order(qt_sel, decreasing=T)]
```
...
order_gender <- sel_com_names[order(unlist(auc_g), decreasing = T)]
densityplot(unlist(auc_g))
sel_com_names[which(auc_g > .85)]
```

Boxplots of these most gender related genes:
```{r, echo = FALSE}
toplot <- norm_data[,c("gender_comp", sel_com_names[which(auc_g > .85)])]
for(i in seq_along(sel_com_names[which(auc_g > .85)])){
  data_tmp <- data.frame(Var = toplot[, i+1],group = toplot[, 1])
  g<- ggplot(data_tmp) + geom_boxplot(aes(x = group, y = Var, col = group)) + 
    geom_jitter(aes(x = group, y = Var, col = group), shape=16, position=position_jitter(0.2)) +
    theme_minimal()
...
Of the 310 genes that were selected as informative on tolerance in both cohorts, 
which ones are still high after correcting for age and gender?
```{r, echo = FALSE}
sel_com_names <- selected_genes_qt[sel_common]
...
A few genes (tale of the second "bump" in the density plot) still add 
information on the tolerance even after correction for gender and age:
```{r, echo = FALSE}
names(qt_sel)[which(qt_sel>0.8)]
order_demo_corr <- sel_com_names[order(qt_sel, decreasing=T)]
...
order_gender <- sel_com_names[order(unlist(auc_g), decreasing = T)]
densityplot(unlist(auc_g))
sel_com_names[which(auc_g > .71)]
```

Boxplots of these most gender related genes:
```{r, echo = FALSE}
toplot <- norm_data[,c("gender_comp", sel_com_names[which(auc_g > .71)])]
for(i in seq_along(sel_com_names[which(auc_g > .71)])){
  data_tmp <- data.frame(Var = toplot[, i+1],group = toplot[, 1])
  g<- ggplot(data_tmp) + geom_boxplot(aes(x = group, y = Var, col = group)) + 
    geom_jitter(aes(x = group, y = Var, col = group), shape=16, position=position_jitter(0.2)) +
    theme_minimal()
...
Of the 104 genes that were selected as informative on tolerance in both cohorts, 
which ones are still high after correcting for age and gender?
```{r, echo = FALSE}
sel_com_names <- selected_genes_qt[sel_common]
...
A few genes (tale of the second "bump" in the density plot) still add information on the 
...
order_gender <- sel_com_names[order(unlist(auc_g), decreasing = T)]
densityplot(unlist(auc_g))
sel_com_names[which(auc_g > .9)]
```

Boxplots of these most gender related genes:
```{r, echo = FALSE}
toplot <- norm_data[,c("gender_comp", sel_com_names[which(auc_g > .9)])]
for(i in seq_along(sel_com_names[which(auc_g > .9)])){
  data_tmp <- data.frame(Var = toplot[, i+1],group = toplot[, 1])
  g<- ggplot(data_tmp) + geom_boxplot(aes(x = group, y = Var, col = group)) + 
    geom_jitter(aes(x = group, y = Var, col = group), shape=16, position=position_jitter(0.2)) +
    theme_minimal()
...
order_age <- sel_com_names[order(unlist(auc_a))]
densityplot(unlist(auc_a))
# which(auc_a<10000000)
```

We can plot the genes that seemed to be most linked to the age of the recipients:
```{r, echo = FALSE}
toplot <- norm_data[,c("age_recip", "group", sel_com_names[which(auc_a < 10000000)])]
toplot$age_recip <- round(toplot$age_recip/365)
for(i in seq_along(sel_com_names[which(auc_a < 10000000)])){
  data_tmp <- data.frame(Var = toplot[, i+2],age_recip = toplot[, 1], group = toplot[,2])
  g<- ggplot(data_tmp, aes(x = age_recip, y = Var)) + 
    geom_point(aes(x = age_recip, y = Var, col = as.factor(group))) + geom_smooth(method = "loess") +
    theme_minimal()
